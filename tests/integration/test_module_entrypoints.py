...
@pytest.mark.parametrize(
    ("statement", "expected"),
    [
        ("import document_bundle; print(document_bundle.__version__)", "0.1.0"),
        (
            "from document_intelligence import generate_chunks; "
            "print(generate_chunks.__name__)",
            "generate_chunks",
        ),
        (
            "from compat.structured_content import ensure_structured_content; "
            "print(ensure_structured_content.__name__)",
            "ensure_structured_content",
        ),
