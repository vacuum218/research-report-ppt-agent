...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="导出 PowerPoint 模板每页的对象名称和结构信息。"
    )
    parser.add_argument("pptx", type=Path, help="要检查的 .pptx 文件路径")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        help="JSON输出路径；默认与PPTX同目录，文件名为 <stem>_objects.json",
    )
    parser.add_argument(
        "--text-preview-length",
        type=int,
        default=120,
        help="文本预览的最大字符数，默认120",
    )
    parser.add_argument(
        "--no-text-preview",
        action="store_true",
        help="不在JSON中导出文本预览，只导出文本长度",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
