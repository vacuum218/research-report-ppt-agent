...
Slide Outline JSON 是研报内容语义层的数据契约，负责回答“这一页讲什么”。它由 DocumentBundle 上的 LLM Slide Planning 生成，但不承担图表数据、模板映射或 PPT 渲染职责。
...
```text
DocumentBundle
  → Document Intelligence：确定性结构与 evidence
  → Context Compression：仅运行时 memory
...
`section_ref` 引用 DocumentBundle 中已有的 section ID。`evidence_refs` 使用
`{"kind":"block|table|figure","id":"原生ID"}` 引用事实证据。二者保持可选以兼容旧
Outline，但新生成的 section/content 页面会执行强制的 bundle 后置校验。
...
- `investment_risk`
- `summary`
- `figure_page`（原始 PDF figure 的独立保真页）

除专用的 `figure_page` 外，不得使用 `chart`、`table`、`two_column`、`content`
等布局或页面形式作为 `slide_type`。

`figure_page` 是当前阶段唯一的原始图片迁移契约：`page_role=content`，
`bullet_points=[]`，`visual_candidates=[]`，并且 `evidence_refs` 只能包含一个
`{"kind":"figure","id":"..."}`。多个 figure 必须拆分为多页并保持 PDF 顺序。
...
| 字段或文件 | 回答的问题 |
|---|---|
| `slide_type` | 这一页讲什么业务主题，或是否为专用 `figure_page`？ |
