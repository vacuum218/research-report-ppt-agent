...
`figure_page` 固定解析到 `image_only_layout`。该布局复用模板第 9 页的基础装饰，
但正文区域只建立一个 `image_slot`，每页必须绑定且只能绑定一个原始 PDF figure。
图片在槽位内等比缩放，不裁切、不拉伸；资源路径相对于 Renderer 的
`asset_root`（DocumentBundle 目录）解析。
